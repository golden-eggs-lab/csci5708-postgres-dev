 ...
 * Global counters — defined here, extern'd in nbtinsert.c and nbtree.c
 * --------------------------------------------------------------------------- */
bool			btree_lab_stats = false;		/* GUC flag: controlled in nbtree.c */
	...
	*bufP = _bt_getroot(rel, heaprel, access);
	...
	for (;;)
	{
		...
		*bufP = _bt_moveright(rel, heaprel, key, *bufP, (access == BT_WRITE),
		...
		if (P_ISLEAF(opaque))
		{
			/* Lab 2: count leaf page visit */
			if (btree_lab_stats)
				bt_leaf_visits++;
			break;
		}

		/* Lab 2: count internal page visit */
		if (btree_lab_stats)
			bt_internal_visits++;
