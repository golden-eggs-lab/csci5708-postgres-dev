 ...
 * Counters are defined in nbtsearch.c; we register the GUC here.
 * --------------------------------------------------------------------------- */
extern bool		btree_lab_stats;
extern int		bt_internal_visits;
extern int		bt_leaf_visits;
extern int		bt_splits;

/*
 * _bt_lab_register_guc — called once at first btinsert/btbeginscan to
 * register the custom GUC.  Using a static flag avoids repeated registration.
 */
static void
_bt_lab_register_guc(void)
{
	...
	DefineCustomBoolVariable(
		...
		"Enable B-tree instrumentation counters (Lab 2).",
		NULL,
		&btree_lab_stats,
		false,
		PGC_USERSET,
		 ...
		 IndexInfo *indexInfo)
{
	...
	/* Lab 2: register GUC on first call */
	_bt_lab_register_guc();

	/* Lab 2: reset counters before this insert */
	bt_internal_visits = 0;
	bt_leaf_visits = 0;
	bt_splits = 0;
	...
	/* Lab 2: print stats after insert */
	if (btree_lab_stats)
...
btbeginscan(Relation rel, int nkeys, int norderbys)
{
	...
	/* Lab 2: register GUC on first call, reset counters for this scan */
	_bt_lab_register_guc();
	bt_internal_visits = 0;
	bt_leaf_visits = 0;
	bt_splits = 0;
...
btendscan(IndexScanDesc scan)
{
	...
	/* Lab 2: print stats at end of scan */
	if (btree_lab_stats)
		elog(INFO, "BTREE_LAB_STATS: internal=%d leaf=%d splits=%d",
			 bt_internal_visits, bt_leaf_visits, bt_splits);

	/* Lab 2: reset counters after printing */
	bt_internal_visits = 0;
	bt_leaf_visits = 0;
	bt_splits = 0;
