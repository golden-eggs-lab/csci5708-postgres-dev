...
_dml_lab_register_guc()
{
    ...
    DefineCustomBoolVariable(
        "dml_lab.enable_logging",
        "Enable DML Logging",
        NULL,
        &logging_enabled, /* pointer to your bool flag */
        false,            /* default OFF */
        PGC_USERSET,
        0,
        NULL, NULL, NULL);

    MarkGUCPrefixReserved("dml_lab");
}

/*
 * ============================================================
 * TODO 1 — DML Lab: Add include, GUC variable, and registration
 * ============================================================
 *
 * (a) Add:   #include "utils/guc.h"
 *
 * (b) Declare a file-scope bool variable for the GUC flag
 *     (default false) and a bool for tracking registration.
 *
 * (c) Write a static helper function _dml_lab_register_guc() that:
 *     - returns immediately if already registered
 *     - calls DefineCustomBoolVariable() to create "dml_lab.enable_logging"
 *       (PGC_USERSET, default false, linked to your flag variable)
 *     - calls MarkGUCPrefixReserved("dml_lab")
 *
 * Hint: the pattern is the same as Lab 2's _bt_lab_register_guc().
 */
           ...
           ResultRelInfo **insert_destrel)
{
    ...
    else
    {
        ...
        if (mtstate->operation == CMD_UPDATE)
            ...
            wco_kind = (mtstate->mt_merge_action->mas_action->commandType == CMD_UPDATE) ? WCO_RLS_UPDATE_CHECK : WCO_RLS_INSERT_CHECK;
        else
        ...
        else
        {
            ...
            /*
             * ============================================================
             * TODO 2 — DML Lab: INSERT logging
             * ============================================================
             *
             * After the tuple has been inserted, log it if the first column
             * is an odd integer.
             *
             * Steps:
             *   1. Call _dml_lab_register_guc()
             *   2. If the GUC flag is on:
             *      a. Use slot_getattr(slot, 1, &isnull) to read column 1
             *      b. If not null, convert to int32 with DatumGetInt32()
             *      c. If the value is odd (val % 2 != 0), call:
             *           elog(INFO, "DML_LAB_LOG: INSERT val=%d", val);
             *
             * The variable `slot` holds the just-inserted tuple.
             */
            _dml_lab_register_guc();
            if (logging_enabled)
            {
                ...
                if (!isnull)
                {
                    ...
                    if (v % 2 != 0) /* odd integer check */
                        elog(INFO, "DML_LAB_LOG: INSERT val=%d", v);
                }
            }
            /* insert index entries for tuple */
           ...
           TupleTableSlot **epqreturnslot)
{
    ...
    else
    {
        ...
        /*
         * ============================================================
         * TODO 3 — DML Lab: DELETE logging
         * ============================================================
         *
         * Before the tuple is deleted, fetch the OLD tuple and log it
         * if its first column is an even integer.
         *
         * Steps:
         *   1. Call _dml_lab_register_guc()
         *   2. If the GUC flag is on AND tupleid is not NULL:
         *      a. Create a temporary slot:
         *           TupleTableSlot *fetchslot;
         *           fetchslot = table_slot_create(resultRelationDesc, NULL);
         *      b. Fetch the old tuple into it:
         *           table_tuple_fetch_row_version(resultRelationDesc,
         *               tupleid, SnapshotAny, fetchslot)
         *         (returns bool — true if the tuple was found)
         *      c. If found, use slot_getattr(fetchslot, 1, &isnull)
         *         to read the first column; convert with DatumGetInt32()
         *      d. If the value is even (val % 2 == 0), call:
         *           elog(INFO, "DML_LAB_LOG: DELETE val=%d", val);
         *      e. Drop the slot: ExecDropSingleTupleTableSlot(fetchslot);
         *
         * The variable `resultRelationDesc` is the relation (table).
         * The variable `tupleid` points to the row to be deleted.
         * SnapshotAny is defined in utils/snapmgr.h (already included).
         */
        _dml_lab_register_guc();
        if (logging_enabled && ItemPointerIsValid(tupleid))
        {
            TupleTableSlot *fetchslot;
            fetchslot = table_slot_create(resultRelationDesc, NULL);
            ...
            {
                bool isnull;
                Datum val = slot_getattr(fetchslot, 1, &isnull); /* 1 = first column */

                if (!isnull)
                {
                    int32 v = DatumGetInt32(val);
                    if (v % 2 == 0) /* even integer check */
                        elog(INFO, "DML_LAB_LOG: DELETE val=%d", v);
                }
            }
                   ...
                   HeapTuple oldtuple, TupleTableSlot *slot)
{
    ...
    ExecARUpdateTriggers(context->estate, resultRelInfo,
                         ...
                         tupleid, oldtuple, slot,
                         recheckIndexes,
                         mtstate->operation == CMD_INSERT ? mtstate->mt_oc_transition_capture : mtstate->mt_transition_capture,
                         false);
           ...
           TupleTableSlot *slot, bool canSetTag)
{
    ...
    redo_act:
        ...
        /*
         * ============================================================
         * TODO 4 — DML Lab: UPDATE logging
         * ============================================================
         *
         * After a successful update, log the operation if the OLD or
         * NEW first-column value is divisible by 3.
         *
         * Steps:
         *   1. Call _dml_lab_register_guc()
         *   2. If the GUC flag is on:
         *      a. Read old value: slot_getattr(oldSlot, 1, &old_isnull)
         *      b. Read new value: slot_getattr(slot, 1, &new_isnull)
         *      c. If both are non-null, convert with DatumGetInt32()
         *      d. If old_val % 3 == 0  OR  new_val % 3 == 0, call:
         *           elog(INFO, "DML_LAB_LOG: UPDATE old_val=%d new_val=%d",
         *                old_val, new_val);
         *
         * `oldSlot` has the tuple BEFORE the update.
         * `slot`    has the tuple AFTER the update.
         * Both are TupleTableSlot* parameters already available in
         * this function (ExecUpdate).
         */
        _dml_lab_register_guc();
        if (logging_enabled)
        {
            bool isnullOld;
            bool isnullNew;
            Datum valOld = slot_getattr(oldSlot, 1, &isnullOld); /* 1 = first column */
            Datum valNew = slot_getattr(slot, 1, &isnullNew);

            if (!isnullOld && !isnullNew)
            {
                int32 v1 = DatumGetInt32(valOld);
                int32 v2 = DatumGetInt32(valNew);
                if (v1 % 3 == 0 || v2 % 3 == 0)
                {
                    elog(INFO, "DML_LAB_LOG: UPDATE old_val=%d new_val=%d",
                         v1, v2);
...
lmerge_matched:
    ...
    foreach (l, actionStates)
    {
        ...
        {
            ExecWithCheckOptions(commandType == CMD_UPDATE ? WCO_RLS_MERGE_UPDATE_CHECK : WCO_RLS_MERGE_DELETE_CHECK,
                                 resultRelInfo,
                                 resultRelInfo->ri_oldTupleSlot,
            ...
            operation == CMD_MERGE)
        {
            ...
            else if (AttributeNumberIsValid(resultRelInfo->ri_RowIdAttNo))
            {
                ...
                oldtupdata.t_tableOid =
                    (relkind == RELKIND_VIEW) ? InvalidOid : RelationGetRelid(resultRelInfo->ri_RelationDesc);
