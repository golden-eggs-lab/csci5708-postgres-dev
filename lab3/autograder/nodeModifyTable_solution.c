...
/* ---- DML Lab: GUC and helper ---- */
static bool dml_lab_logging = false;
static bool dml_lab_guc_registered = false;

static void
_dml_lab_register_guc(void)
{
	if (dml_lab_guc_registered)
		return;
	dml_lab_guc_registered = true;

	DefineCustomBoolVariable(
		"dml_lab.enable_logging",
		"Enable DML operation logging for lab",
		NULL,
		&dml_lab_logging,
		false,
		PGC_USERSET,
		   ...
		   TupleTableSlot **epqreturnslot)
{
	...
	else
	{
		...
		/* ---- DML Lab: DELETE logging ---- */
		_dml_lab_register_guc();
		if (dml_lab_logging && tupleid != NULL)
		{
			...
			{
				bool	isnull;
				Datum	val = slot_getattr(fetchslot, 1, &isnull);
...
redo_act:
		...
		/* ---- DML Lab: UPDATE logging ---- */
		_dml_lab_register_guc();
		if (dml_lab_logging)
		{
			bool	old_isnull, new_isnull;
			Datum	old_val = slot_getattr(oldSlot, 1, &old_isnull);
			Datum	new_val = slot_getattr(slot, 1, &new_isnull);

			if (!old_isnull && !new_isnull)
			{
				int32	ov = DatumGetInt32(old_val);
				int32	nv = DatumGetInt32(new_val);

				if (ov % 3 == 0 || nv % 3 == 0)
					elog(INFO, "DML_LAB_LOG: UPDATE old_val=%d new_val=%d", ov, nv);
			}
		}
	}
