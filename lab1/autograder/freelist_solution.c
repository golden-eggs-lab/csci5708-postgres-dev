...
/* Global timestamp counter for 2nd-LRU */
int lru_global_timestamp = 1;
...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	...
	/* Nothing on the freelist, so run the 2nd-LRU algorithm */
	{
		int round;
		for (round = 0; round < 10; round++)
		{
			int least_idx = -1;
			int second_least_idx = -1;
			int least_ts = INT_MAX;
			int second_least_ts = INT_MAX;
			int candidate_count = 0;
			int i;

			/* Scan all buffers to find candidates (refcount == 0) */
			for (i = 0; i < NBuffers; i++)
			{
				BufferDesc *candidate = GetBufferDescriptor(i);
				uint32 cand_state = pg_atomic_read_u32(&candidate->state);

				if (BUF_STATE_GET_REFCOUNT(cand_state) == 0)
				{
					candidate_count++;
					if (candidate->lru_timestamp < least_ts)
					{
						/* Previous least becomes second least */
						second_least_ts = least_ts;
						second_least_idx = least_idx;
						/* New least */
						least_ts = candidate->lru_timestamp;
						least_idx = i;
					}
					else if (candidate->lru_timestamp < second_least_ts)
					{
						second_least_ts = candidate->lru_timestamp;
						second_least_idx = i;
					}
				}
			}
			...
			/* Print candidate timestamps */
			{
				...
				for (i = 0; i < NBuffers; i++)
				{
					...
					if (BUF_STATE_GET_REFCOUNT(cand_state) == 0)
					{
						if (!first)
							printf(", ");
						printf("%d", candidate->lru_timestamp);
						first = 0;
					}
				}
				printf("\n");
			...
			/* If only 1 candidate, evict it */
			if (candidate_count == 1)
			{
				buf = GetBufferDescriptor(least_idx);
				...
				UnlockBufHdr(buf, local_buf_state);
				continue; /* buffer got pinned, try again */
			}

			/* Otherwise evict the 2nd least recently used */
			buf = GetBufferDescriptor(second_least_idx);
			printf("Replaced buffer: %d\n", buf->lru_timestamp);
			local_buf_state = LockBufHdr(buf);
			if (BUF_STATE_GET_REFCOUNT(local_buf_state) == 0)
			{
			...
			UnlockBufHdr(buf, local_buf_state);
			/* buffer got pinned between scan and lock, try again */
		}

		/* After 10 rounds, abort */
